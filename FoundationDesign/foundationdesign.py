...
class PadFoundation:
    ...
    def bearing_pressure_check_sls(self):
        ...
        return {
            ...
            "q_allow": self.soil_bearing_capacity,
            "status": status,
        }
    ...
    def uls_analysis_trace(self):
        """Return the intermediate values used by the ULS pressure model.

        The returned mapping is intended for calculation reports and other
        clients that need traceability without reimplementing the engineering
        equations outside this package.
        """
        fdn_pressure, soil_pressure = self.foundation_loads(
            self.foundation_thickness * 1000,
            self.soil_depth_abv_foundation * 1000,
            self.soil_unit_weight,
            self.concrete_unit_weight,
        )
        area = self.area_of_foundation()
        design_axial_load = self.total_force_Z_dir_uls()
        total_moment_x = self.total_moments_X_direction_uls()
        total_moment_y = self.total_moments_Y_direction_uls()
        eccentricity_x = self.eccentricity_X_direction_uls() / 1000
        eccentricity_y = self.eccentricity_Y_direction_uls() / 1000
        corner_pressures = self.pad_base_pressures_uls()
        line_reactions_x = self.base_pressure_rate_of_change_X()
        line_reactions_y = self.base_pressure_rate_of_change_Y()

        return {
            "combination": {
                "standard": "EN 1990:2002+A1:2005",
                "clause": "6.4.3.2",
                "expression": "6.10",
                "annex_table": "A1.2(B)",
                "permanent_factor": self.uls_strength_factor_permanent,
                "imposed_factor": self.uls_strength_factor_imposed,
            },
            "geometry": {
                "length": self.foundation_length,
                "width": self.foundation_width,
                "area": area,
                "thickness": self.foundation_thickness,
                "column_centre_x": self.col_pos_xdir,
                "column_centre_y": self.col_pos_ydir,
                "column_length": self.column_length,
                "column_width": self.column_width,
            },
            "actions": {
                "permanent_axial_load": self.permanent_axial_load,
                "imposed_axial_load": self.imposed_axial_load,
                "permanent_moment_x": self.permanent_moment_xdir,
                "imposed_moment_x": self.imposed_moment_xdir,
                "permanent_moment_y": self.permanent_moment_ydir,
                "imposed_moment_y": self.imposed_moment_ydir,
                "permanent_horizontal_x": self.permanent_horizontal_load_xdir,
                "imposed_horizontal_x": self.imposed_horizontal_load_xdir,
                "permanent_horizontal_y": self.permanent_horizontal_load_ydir,
                "imposed_horizontal_y": self.imposed_horizontal_load_ydir,
            },
            "foundation_pressures": {
                "concrete": fdn_pressure,
                "soil": soil_pressure,
            },
            "resultants": {
                "design_axial_load": design_axial_load,
                "total_moment_x_about_origin": total_moment_x,
                "total_moment_y_about_origin": total_moment_y,
                "eccentricity_x": eccentricity_x,
                "eccentricity_y": eccentricity_y,
                "uniform_pressure": design_axial_load / area,
            },
            "corner_pressures": {
                "q1": corner_pressures[0],
                "q2": corner_pressures[1],
                "q3": corner_pressures[2],
                "q4": corner_pressures[3],
            },
            "edge_line_reactions": {
                "x": {
                    "start": line_reactions_x[0],
                    "end": line_reactions_x[1],
                },
                "y": {
                    "start": line_reactions_y[0],
                    "end": line_reactions_y[1],
                },
            },
        }
...
class padFoundationDesign(PadFoundation):
    ...
    def get_design_moment_trace(self, direction):
        """Return the complete strip-analysis trace for a column-face moment.

        Parameters
        ----------
        direction : str
            ``"X"`` for the footing-length strip or ``"Y"`` for the
            footing-width strip.

        Returns
        -------
        dict
            ULS pressure-model data and the trapezoidal strip-load
            integration used to obtain the moment at the positive column face.
        """
        direction = str(direction).upper()
        if direction not in ("X", "Y"):
            raise ValueError("direction must be 'X' or 'Y'")

        trace = self.PadFoundation.uls_analysis_trace()
        geometry = trace["geometry"]
        pressures = trace["foundation_pressures"]
        reactions = trace["edge_line_reactions"][direction.lower()]

        if direction == "X":
            axis_length = geometry["length"]
            strip_width = geometry["width"]
            face_coordinate = (
                geometry["column_centre_x"] + geometry["column_length"] / 2
            )
            method_moment = self.get_design_moment_X()
        else:
            axis_length = geometry["width"]
            strip_width = geometry["length"]
            face_coordinate = (
                geometry["column_centre_y"] + geometry["column_width"] / 2
            )
            method_moment = self.get_design_moment_Y()

        outstand = axis_length - face_coordinate
        start_reaction = reactions["start"]
        edge_reaction = reactions["end"]
        face_reaction = start_reaction + (
            (edge_reaction - start_reaction)
            * face_coordinate
            / axis_length
        )
        factored_dead_line_load = (
            trace["combination"]["permanent_factor"]
            * (pressures["concrete"] + pressures["soil"])
            * strip_width
        )
        net_face_load = face_reaction - factored_dead_line_load
        net_edge_load = edge_reaction - factored_dead_line_load
        integrated_moment = (
            outstand ** 2 * (net_face_load + 2 * net_edge_load) / 6
        )
        ...
        return {
            **trace,
            "strip": {
                ...
                "outstand": outstand,
                "start_soil_reaction": start_reaction,
                "face_soil_reaction": face_reaction,
                "edge_soil_reaction": edge_reaction,
                "factored_dead_line_load": factored_dead_line_load,
                "net_face_load": net_face_load,
                "net_edge_load": net_edge_load,
                "integrated_face_moment": integrated_moment,
                "analysis_method_moment": method_moment,
            },
        }
