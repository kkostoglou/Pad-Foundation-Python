...
After a successful design, this tab provides separate views for the reinforcement
layout, bending-moment diagrams in the X and Y directions, and shear-force diagrams
in the X and Y directions. The action diagrams use the analysed curves generated by
the FoundationDesign calculation engine. Each diagram includes an aligned pad and
column elevation so the curve can be related to the physical foundation and column
position. Values are marked on the curve at the left and right pad ends and at both
column faces. Moments are displayed in kNm, shear forces in kN, and positions in m;
the underlying SI results are converted from N.m and N for display. Changing an
input clears the diagrams until the design is run again.
