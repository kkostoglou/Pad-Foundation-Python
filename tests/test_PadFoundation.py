import unittest
from FoundationDesign import PadFoundation
...
class PadFoundationTestCase(unittest.TestCase):
    ...
    def test_uls_analysis_trace(self):
        trace = self.pad_foundation.uls_analysis_trace()

        self.assertEqual(trace["combination"]["expression"], "6.10")
        self.assertEqual(trace["resultants"]["design_axial_load"], 1726.956)
        self.assertEqual(
            trace["resultants"]["total_moment_x_about_origin"], 3303.183
        )
        self.assertEqual(
            trace["corner_pressures"],
            {"q1": 129.863, "q2": 129.863, "q3": 189.944, "q4": 189.944},
        )
        self.assertEqual(
            trace["edge_line_reactions"]["x"],
            {"start": 389.589, "end": 569.832},
        )
