...
class PadFoundationDesignTestCase(unittest.TestCase):
    ...
    def test_design_moment_trace(self):
        x_trace = self.pad_foundation_design.get_design_moment_trace("X")
        y_trace = self.pad_foundation_design.get_design_moment_trace("Y")

        self.assertEqual(x_trace["strip"]["direction"], "X")
        self.assertEqual(y_trace["strip"]["direction"], "Y")
        self.assertAlmostEqual(
            x_trace["strip"]["integrated_face_moment"],
            x_trace["strip"]["analysis_method_moment"],
            places=2,
        )
        self.assertAlmostEqual(
            y_trace["strip"]["integrated_face_moment"],
            y_trace["strip"]["analysis_method_moment"],
            places=2,
        )
